...
df <- read_csv("data13_final_ml_star.csv", show_col_types = FALSE)

target_var <- "satisfied_stars" 

df[[target_var]] <- factor(
  ifelse(df[[target_var]] == 1, "Yes", "No"),
  levels = c("No", "Yes"))

# Reduced dataset for Logit / NB to handle multicollinearity
df_reduced <- df %>%
  dplyr::select(-avg_temp_lag1, -avg_temp_roll3, -PRCP_avg_lag1, -PRCP_avg_roll3)


logit_final <- glm(satisfied_stars ~ ., data = df_reduced, family = "binomial")
...
# -------------------------
# Preprocessing Recipes (Normal & SMOTE)

base_rec_full <- recipe(as.formula(paste(target_var, "~ .")), data = train_full_raw) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_zv(all_predictors())

# Reduced Pipeline (for Logit/NB)
base_rec_red <- recipe(as.formula(paste(target_var, "~ .")), data = train_red_raw) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_zv(all_predictors())
...
# Processed datasets
# Normal Sampling
prep_full_norm <- prep(base_rec_full); train_pp_full_norm <- juice(prep_full_norm)
prep_red_norm  <- prep(base_rec_red);  train_pp_red_norm  <- juice(prep_red_norm)

# SMOTE Sampling (50/50 balance)
prep_full_smote <- base_rec_full %>% step_smote(!!sym(target_var), over_ratio = 0.5) %>% prep()
train_pp_full_smote <- juice(prep_full_smote)

prep_red_smote <- base_rec_red %>% step_smote(!!sym(target_var), over_ratio = 0.5) %>% prep()
train_pp_red_smote <- juice(prep_red_smote)

# Test Sets
test_pp_full <- bake(prep_full_norm, new_data = test_full_raw)
test_pp_red  <- bake(prep_red_norm, new_data = test_red_raw)
...
train_predict <- function(model, train_df, test_df, target) {
  ...
  if (model == "Logit") {
    ...
    p_tr <- predict(fit, train_df, type = "response"); p_ts <- predict(fit, test_df, type = "response")
    
  } else if (model == "Tree") {
    fit <- rpart(as.formula(paste(target, "~ .")), data = train_df)
    p_tr <- predict(fit, train_df)[,"Yes"]; p_ts <- predict(fit, test_df)[,"Yes"]
    
  } else if (model == "Bagging") {
    fit <- ipred::bagging(as.formula(paste(target, "~ .")), data = train_df, nbagg = 25)
    p_tr <- predict(fit, train_df, type = "prob")[,"Yes"]; p_ts <- predict(fit, test_df, type = "prob")[,"Yes"]
    
  } else if (model == "RF") {
    fit <- ranger(x = X_train, y = y_train, probability = TRUE, num.trees = 100, num.threads = 1)
    p_tr <- predict(fit, X_train)$predictions[,"Yes"]; p_ts <- predict(fit, X_test)$predictions[,"Yes"]
    
  } else if (model == "XGB") {
    fit <- xgboost(data = X_train, label = as.numeric(y_train=="Yes"), nrounds = 30, objective = "binary:logistic", verbose=0, nthread = 1)
    p_tr <- predict(fit, X_train); p_ts <- predict(fit, X_test)
    
  } else if (model == "SVM") {
    fit <- LiblineaR(X_train, y_train, type = 7)
    p_tr <- predict(fit, X_train, proba = TRUE)$probabilities[,"Yes"]; p_ts <- predict(fit, X_test, proba = TRUE)$probabilities[,"Yes"]
    ...
    p_tr <- predict(fit, train_df, type = "raw")[,"Yes"]; p_ts <- predict(fit, test_df, type = "raw")[,"Yes"]
    
  } else if (model == "NN") {
    fit <- nnet::nnet(as.formula(paste(target, "~ .")), data = train_df, size = 3, maxit = 50, trace = FALSE)
    p_tr <- as.vector(predict(fit, train_df, type = "raw")); p_ts <- as.vector(predict(fit, test_df, type = "raw"))
  }
...
results_all <- foreach(m = model_list, .packages = c("caret","pROC","ranger","xgboost","rpart","ipred","LiblineaR","e1071","nnet","dplyr")) %dopar% {
  ...
  # Normal Sampling
  res_n <- train_predict(m, if(is_red) train_pp_red_norm 
                         else train_pp_full_norm, 
                         if(is_red) test_pp_red 
                         else test_pp_full, target_var)
  
  met_n <- compute_metrics(if(is_red) test_pp_red[[target_var]] 
                           else test_pp_full[[target_var]],
                           res_n$p_test, res_n$threshold, m, "Normal")
  
  # SMOTE Sampling
  res_s <- train_predict(m, if(is_red) train_pp_red_smote
                         else train_pp_full_smote, 
                         if(is_red) test_pp_red 
                         else test_pp_full, target_var)
  
  met_s <- compute_metrics(if(is_red) test_pp_red[[target_var]]
                           else test_pp_full[[target_var]], 
...
roc_df <- bind_rows(lapply(names(predictions_normal), function(m) {
  # Use test_full_raw and the dynamic target_var
...
# We find the best threshold based on training results
